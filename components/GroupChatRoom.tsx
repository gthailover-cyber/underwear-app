...
const GroupChatRoom: React.FC<GroupChatRoomProps> = ({ room, language, onBack, currentUser, walletBalance, onUseCoins, onOpenWallet }) => {
  ...
  const members = MOCK_PEOPLE.slice(0, 15); // Take first 15 for demo
         ...
         <div className="flex bg-gray-800 p-1 rounded-lg">
            <button 
              ...
              className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${
                 activeTab === 'chat' ? 'bg-gray-700 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'
              }`}
            >
               Chat
            </button>
            <button 
              ...
              className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${
                 activeTab === 'members' ? 'bg-gray-700 text-white shadow-sm' : 'text-gray-500 hover:text-gray-300'
              }`}
            >
               Members
            </button>
